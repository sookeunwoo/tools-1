...
| 컬렉션을 항목이 아니라 **별도 파일**에 둔다 | 화면 정리 정보가 설정 스키마에 섞이면 두 관심사가 같이 흔들린다. secret을 옮기는 데 age 개인키가 필요해지는 것도 과한 대가다 |
| 컬렉션 삭제가 key를 **지우지 않는다** | 정리 도구가 데이터를 지우면, 되돌릴 수 없는 실수를 한 번의 클릭으로 만들 수 있다 |
