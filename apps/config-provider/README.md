...
`dkc ui` → `새 항목` → key·저장 위치(public/secret)·별칭·설명을 채우고 env별 초기 값까지
한 번에 입력. 값은 표에서 그 자리에 다시 편집할 수 있다.
...
배치는 Postman 워크스페이스를 따른다: 상단 툴바 · 좌측 탐색기 · 가운데 표 · 우측 인스펙터.
**값은 예외 없이 표로 그린다.** 기본 화면은 항목 × env 매트릭스이고, 인스펙터의 값·참조·
메타·별칭·CLI도 전부 표다. 카드보다 표가 같은 높이에서 더 많은 값을 같은 자리에 정렬해 준다.
...
- prod / dev / stg를 한 행에서 나란히 본다. env 알약으로 한 env만 남길 수도 있다
...
- 좌측 탐색기에서 env · 종류 · 상태(미설정/오류/참조 사용)로 좁히고, `/`로 검색에 바로 간다
