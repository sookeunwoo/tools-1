...
| **M1** `context-pack` · `repo-map` · `trace-flow` | ✅ **완료** |
| M2~M6 | §14 |
...
**M1 v0 — 구현 완료.** `repo-map`이 아직 없어 git과 파일 구조에서 확인 가능한 사실만 담는다:
변경·신규 파일과 라인 수, 브랜치·기준·최근 커밋, 빌드/테스트 명령, 테스트 파일 존재 여부(이름 규칙).
호출 관계·트랜잭션 경계는 추측하지 않고 `unresolved`에 명시한다. 근거 인용은 M2에서 붙인다.

> **도그푸딩에서 잡은 버그 3건** (자기 자신에게 돌려서 발견):
> ① untracked 신규 파일이 `git diff`에 안 잡혀 브리핑에서 통째로 누락 — 작업 중 새로 만드는 파일이
>    가장 중요한데 안 보였다. ② `git diff --no-index`가 차이 발생 시 exit 1을 내서 신규 파일
>    라인 수가 항상 0. ③ `.json`·`.md`가 테스트 갭에 섞여 DoD에 잘못된 항목 생성 →
>    소스 확장자로 한정. 전부 회귀 테스트로 고정했다.
...
#### ADR-002 보충 (M1 구현 중 결정)

초안은 tree-sitter(WASM)를 1단계로 잡았지만, **구현 시점에 한 단계 아래에서 시작했다.**

이유: tree-sitter는 npm 의존성이다. README·AGENTS.md가 **"`npm install` 불필요, 의존성 0"**을
계약으로 내걸고 있고, 그게 제약 2(에이전트가 툴을 고칠 수 있다)를 떠받치는 축이다.
M1에서 그 계약을 깨는 건 아키텍처 후퇴다. 그리고 ADR-004가 이미 "측정해서 승급한다" 구조라
같은 방식을 한 칸 아래에도 적용하는 게 일관된다.

| 단계 | 방식 | 의존성 | 상태 |
|---|---|---|---|
| **1** | **어휘 스캐너** — 주석·문자열 제거 후 선언 구조 파싱 | **0** | ✅ M1에서 채택 |
| 2 | tree-sitter (WASM) | npm(순수 WASM) | 1단계 정확도 미달 시 |
| 3 | Kotlin Analysis API (JVM 사이드카) | JVM | ADR-004 트리거 |

1단계로 실제 확인한 것: 주석·문자열 오탐 제거, Spring 어노테이션 경로 조합,
DI 필드 기반 수신자 해석, 인터페이스 다중 구현 분기, `@Transactional` 경계 추적,
`@Table` 테이블명 추출. 못 하는 것: 타입 추론, 지역 변수 수신자, 리플렉션·AOP.
...
| ADR | 선택 | 핵심 근거 | 트레이드오프 |
|---|---|---|---|
| **002** 파싱 | **어휘 스캐너(의존성 0) → tree-sitter → JVM Analysis API 3단 사다리** | 아래 참조 | 단계마다 정확도/의존성 트레이드오프 |
